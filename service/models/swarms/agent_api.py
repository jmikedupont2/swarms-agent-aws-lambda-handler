...
class AgentStore:
    ...
    async def get_agent_metrics(self, agent_id: UUID) -> AgentMetrics:
        ...
        success_rate = metadata['successful_completions'] / metadata['total_completions'] * 100 if metadata['total_completions'] > 0 else 0

        return AgentMetrics(
            total_completions=metadata['total_completions'],
            average_response_time=(sum(response_times) / len(response_times) if response_times else 0),
            error_rate=(metadata['error_count'] / metadata['total_completions'] if metadata['total_completions'] > 0 else 0),
            last_24h_completions=sum(1 for t in response_times if (datetime.utcnow() - t).days < 1),
    ...
    async def process_completion(
        ...
        try:
            ...
            metadata['tokens_per_minute'][current_minute] = metadata['tokens_per_minute'].get(current_minute, 0) + total_tokens
...
class SwarmsAPI:
    ...
    async def _cleanup_old_metrics(self, agent_id: UUID):
        ...
        if metadata:
            ...
            metadata['response_times'] = [t for t in metadata['response_times'] if isinstance(t, (int, float)) and t > cutoff.timestamp()]

            # Clean up old tokens per minute data
            if 'tokens_per_minute' in metadata:
                metadata['tokens_per_minute'] = {k: v for k, v in metadata['tokens_per_minute'].items() if k > cutoff}
